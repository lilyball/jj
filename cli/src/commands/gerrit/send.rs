...
fn calculate_push_remote(
    ...
    // case 2
    if let Ok(remote) = settings.get_string("gerrit.default_remote") {
        if remotes.iter().any(|r| r == Some(&remote)) {
            return Ok(remote);
        }
        return Err(user_error(format!(
...
fn calculate_push_ref(
    ...
    // case 2
    if let Ok(default_for) = settings.get_string("gerrit.default_for") {
...
pub fn cmd_send(ui: &mut Ui, command: &CommandHelper, send: &SendArgs) -> Result<(), CommandError> {
    ...
    let for_remote =
        calculate_push_remote(store, command.settings(), send.remote.clone())?;
    ...
    let new_heads = base_repo
        ...
        .map_err(|e| internal_error_with_message("failed to fetch repo heads for new commits", e))?;
