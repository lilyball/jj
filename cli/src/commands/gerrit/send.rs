...
pub fn cmd_send(ui: &mut Ui, command: &CommandHelper, send: &SendArgs) -> Result<(), CommandError> {
    ...
    let for_remote =
        calculate_push_remote(store, command.settings().raw_config(), send.remote.clone())?;
    let for_branch = calculate_push_ref(command.settings().raw_config(), send.for_.clone())?;
